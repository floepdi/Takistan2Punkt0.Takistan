...
if(!(EQUAL(_level,-1))) then {
    _flag = switch(playerSide) do {case west: {(_level <= (FETCH_CONST(life_copLevel)))}; case independent: {(_level <= (FETCH_CONST(life_medicLevel)))}; case civilian: {(_level <= (FETCH_CONST(life_levelshop)))}; default {true};};
